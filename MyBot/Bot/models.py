...
class TelegramUser(models.Model):
    ...
    telegram_id = models.PositiveBigIntegerField(
        "ID Telegram", null=True, db_index=True, unique=True
    )
    ...
    datetime_add = models.DateTimeField(
        "Время регистрации", auto_now_add=True, blank=True, null=True
    )
    datetime_update = models.DateTimeField(
        "Время последней активности", auto_now=True, blank=True, null=True
    )
    ...
    def __str__(self):
        return f"id: {self.telegram_id} ---- " f"имя: {self.first_name}"
...
class BankCard(models.Model):
    ...
    name_bank = models.CharField(
        max_length=150, blank=True, null=True, unique=True, verbose_name="Банк"
    )
    slug = AutoSlugField(
        populate_from="name_bank",
        max_length=255,
        unique=True,
        db_index=True,
        verbose_name="slug",
    ...
    datetime_add = models.DateTimeField(
        "Время добавления", auto_now_add=True, blank=True, null=True
    )
...
class CardUser(models.Model):
    ...
    datetime_update = models.DateTimeField(
        "Время последнего изменения", auto_now=True, blank=True, null=True
    )
...
class CategoryOperation(models.Model):
    ...
    def __str__(self):
        return f"Категория: {self.name_cat},  " f"{self.type}"


class Recipient(models.Model):
    """Получатели/плательщики"""

    name_recipient = models.CharField(
        max_length=150,
        null=True,
        unique=True,
        verbose_name="Получатель платежа"
    )
    recipient_in_notification = models.CharField(
        max_length=150, blank=True, null=True, verbose_name="Получатель в уведомлении"
    )
    slug = AutoSlugField(
        populate_from="name_recipient",
        max_length=255,
        unique=True,
        db_index=True,
        verbose_name="slug",
...
class OperationUser(models.Model):
    ...
    amount_operation = models.DecimalField(
        "Сумма операции",
        max_digits=10,
        decimal_places=2,
        db_index=True,
    ...
    note_operation = models.CharField(
        max_length=250, blank=True, null=True, verbose_name="Текст уведомления"
    )
    balans = models.DecimalField(
